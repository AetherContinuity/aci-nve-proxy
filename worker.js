...
//   GET /syke        ?entity=Vedenkorkeus&$filter=&$select=&$orderby=&$top=&$skip=   Hydrologiarajapinta (OData)
//   GET /syke/meta   ?entity=Paikka            entiteetin kentät ja tyypit
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (u.pathname === '/syke' || u.pathname === '/syke/') return await handleHydOdata(u.searchParams);
      if (u.pathname === '/syke/meta') return await handleHydMeta(u.searchParams);
...
async function handleVesiraja(url) {
  ...
  if (sub === 'stations') {
    endpoint = `${VESIRAJA_BASE}/stations?api-version=${params.get('v') || '1.0'}`;
  } else if (sub === 'variables') {
    endpoint = `${VESIRAJA_BASE}/variables?api-version=${params.get('v') || '1.0'}`;
    ...
    let q = `api-version=${params.get('v') || '1.0'}&VariableCode=${vc}&DateStart=${start}&DateEnd=${end}`;
...
// ─── Hydrologiarajapinta 1.1 (korjattu v11.1) ───────────────────────
// WaterLevelRegisters-entiteettiä ei ole; oikeat nimet: Paikka, Vedenkorkeus, Virtaama, ...
...
async function handleHydOdata(p) {
  const ent = p.get('entity') || 'Vedenkorkeus';
  if (!HYD_ENT.has(ent)) {
    return new Response(JSON.stringify({ error: `entity not allowed: ${ent}`, allowed: [...HYD_ENT] }), { status: 400, headers: CORS });
  }
  const parts = [];
  for (const k of ['$filter', '$select', '$orderby', '$skip', '$expand']) {
    if (p.get(k)) parts.push(`${k}=${encodeURIComponent(p.get(k))}`);
  }
  const top = Math.min(parseInt(p.get('$top') || '1000', 10) || 1000, 10000);
  parts.push(`$top=${top}`, '$format=json');
  const target = `${HYD}${ent}?${parts.join('&')}`;
  const r = await fetch(target, { headers: { 'Accept': 'application/json', 'User-Agent': 'ACI-HEM/1.1' } });
  const text = await r.text();
  if (!r.ok) {
    return new Response(JSON.stringify({ error: `Hydrologiarajapinta HTTP ${r.status}`, url: target, detail: text.slice(0, 400) }), { status: 502, headers: CORS });
  }
  return new Response(text, { headers: { ...CORS, 'Cache-Control': 'public, max-age=3600', 'X-Upstream-URL': target } });
}

async function handleHydMeta(p) {
  const r = await fetch(`${HYD}$metadata`);
  const x = await r.text();
  const types = {};
  for (const m of x.matchAll(/<EntityType Name="([^"]+)"[^>]*>([\s\S]*?)<\/EntityType>/g)) {
    types[m[1]] = [...m[2].matchAll(/<Property\s+Name="([^"]+)"\s+Type="([^"]+)"/g)].map(a => `${a[1]}:${a[2].replace('Edm.', '')}`);
  }
  const ent = p.get('entity');
  return new Response(JSON.stringify(ent ? { [ent]: types[ent] ?? null } : types), {
