...
//   GET /nve/raw         ?e=SisteUke|MinMaxMedian[&omr=NO&nr=0&uke=37]   NVE:n raakavastaus
...
async function handleNve(env) {
  ...
  try {
    ...
    if (mr.ok) {
      ...
      const med = mmm?.medianFyllingsGrad ?? mmm?.medianFyllingsgrad; // NVE: iso G
...
async function handleNveRaw(env, params) {
  ...
  if (params.get('omr')) arr = arr.filter((x) => x.omrType === params.get('omr'));
  if (params.get('nr')) arr = arr.filter((x) => String(x.omrnr) === params.get('nr'));
  if (params.get('uke')) arr = arr.filter((x) => String(x.iso_uke) === params.get('uke'));
...
async function handleHydOdata(params) {
  ...
  parts.push(`$top=${top}`); // $format ei sallittu, JSON tulee Accept-otsakkeella
